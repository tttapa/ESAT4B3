...
%
% https://www.physionet.org/physiotools/matlab/rddata.m
% -------------------------------------------------------------------------

function [ sfreq, ecg, annotations, annotationSamples ] = readSignalAndAnnotations(path, record, samples)

%------ SPECIFY DATA ------------------------------------------------------
PATH= path; % path, where data are saved
HEADERFILE= strcat(record, '.hea');      % header-file in text format
ATRFILE= strcat(record, '.atr');         % attributes-file in binary format
DATAFILE=strcat(record, '.dat');         % data-file
SAMPLES2READ=samples;         % number of samples to be read
...
clear A;
ATRSAMPLE= (cumsum(ATRTIME));
ATRTIME= (cumsum(ATRTIME))/sfreq;
ind= find(ATRTIME <= TIME(end));
ATRTIMED= ATRTIME(ind);
ATRSAMPLED= ATRSAMPLE(ind);
ANNOT=round(ANNOT);
ANNOTD= ANNOT(ind);

fprintf(1,'\\n$> LOADING ANNOTATION DATA FINISHED \n');

annotationSamples = ATRSAMPLED;
annotations = ANNOTD;
ecg = M;