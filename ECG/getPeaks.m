...
% Go from 2 to length of the input - 1
counter = 2;
...
% Set the first value to the highest value if it is greater than the second
% value
if mem_cur > mem_next
...
% At the end, we need to check if the last value is greater than the
% second-to-last value in the input data
if mem_next > mem_cur
...
if checkDistance && length(tempPeaks) > 0
    ...
    while counter <= totalCount
        ...
        if location2 - location1 < INPUT_minPeakDistance
            ...
            % Save the peak in the next spot
